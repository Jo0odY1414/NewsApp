    ...
    private static List<Story> extractFeatureFromJson(String storyJSON) {
        ...
        try {
            ...
            if (baseJsonResponse.has("response")) {
                ...
                // which represents a list of items (or stories).
                JSONArray storyArray;
                if (secondJsonResponse.has("results"))
                    storyArray = secondJsonResponse.getJSONArray("results");
                else
                ...
                for (int i = 0; i < storyArray.length(); i++) {
                    ...
                    // Get a single story at position i within the list of stores
                    ...
                    // Create a new {@link Story} object with the title, type, sectionName
                    // and storyUrl from the JSON response.
                    Story story = new Story(title,type,sectionName,url);

                    // Extract the value for the key called "webPublicationDate",
                    // when the key is exist add the value to story object.
                    if (currentStory.has("webPublicationDate"))
                        story.setPublicationDate(currentStory.getString("webPublicationDate"));

                    // Extract the value for the key called "authorName",
                    // when the key is exist add the value to story object.
                    if (currentStory.has("authorName"))
                        story.setAuthorName(currentStory.getString("authorName"));
